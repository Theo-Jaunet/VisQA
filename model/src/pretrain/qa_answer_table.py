...
def load_lxmert_qa(path, model, label2ans):
    ...
    loaded = torch.load("%s_LXRT.pth" % path)
    if "model" in loaded:
    ...
    mean_ans_weight = ans_weight.mean(0)
    mean_ans_bias = ans_bias.mean(0)
    import copy
    ...
    for label, ans in label2ans.items():
        ...
        if answer_table.used(new_ans):
            ...
            new_answer_weight[label] = mean_ans_weight
            new_answer_bias[label] = mean_ans_bias
            unload += 1
            new_ans_label.append(label)
